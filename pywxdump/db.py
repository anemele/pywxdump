        ...
        for tbl_name in tabels_all:
            if tbl_name == "MSG":
                MsgSvrIDs = merged_cursor.execute(
                    f"select MsgSvrID from MSG where CreateTime>{CreateTime} and MsgSvrID!=0"
                ).fetchall()
                ...
                for r in cursor.fetchall():
                    ...
                    if len(columns) > 1:
                        ...
                        query_1 = (
                            "select * from " + tbl_name + " where " + columns[0] + "=?"
                        )  # 查询语句 用于判断是否存在
...
def execute_sql(conn: sqlite3.Connection, sql: str, params=None):
    """
    执行给定的SQL语句，返回结果。
    参数：
        - connection： SQLite连接
...
def merge_db(db_path: str | list[str], save_path="merge.db", CreateTime: int = 0):
    """
    合并数据库 会忽略主键以及重复的行。
    """
    if os.path.isdir(save_path):
    ...
    with sqlite3.connect(save_path) as outdb:
        ...
        for database in databases:
            ...
            for table in tables:
                ...
                columns = [
                    i[1] if isinstance(i[1], str) else i[1].decode() for i in columns
                ]
                ...
                if len(out_cursor.fetchall()) < 1:
                    ...
                    for column in col_type:
                        column_name = (
                            ...
                            else column[0].decode()
                        )
                        column_type = (
                            column[1]
                            if isinstance(column[1], str)
                            else column[1].decode()
                        )
